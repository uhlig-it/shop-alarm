...
module MQTT
  class Blink1
    def initialize(broker:, topic:, device:, logger:)
      ...
      @router.add_route('werkstatt/lock') do |t, message|
      ...
      @router.add_route('werkstatt/pir') do |t, message|
        ...
        when 'begin'
          ...
          broker.publish(topic, '{"color":{"r":255,"g":255,"b":0}}')
        when 'end'
          logger.debug(self.class.name) { "Setting LED to grey" }
          device.set_rgb(128, 128, 128)
          broker.publish(topic, '{"color":{"r":128,"g":128,"b":0}}')
        end
      end
    end
